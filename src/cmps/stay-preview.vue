...
<script>
import priceRangeFilter from './filters/price-range-filter.vue';
import priceRange from './filters/price-range-filter.vue';
...
export default {
  ...
  computed: {
    ...
    likedStay() {
      ...
      if (this.stay.likedByUsers) {
        const idxLikedBy = this.stay.likedByUsers.findIndex(
          (id) => id === this.userId
        );
